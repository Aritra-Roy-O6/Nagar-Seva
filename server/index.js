...
app.post('/api/auth/admin/register', async (req, res) => {
  ...
  if (!name || !email || !password || !districtId || !secretKey || !role)
...
app.post('/api/reports', auth, reportLimiter, async (req, res) => {
  ...
  try {
    ...
    if (existingReportResult.rows.length > 0) {
      ...
      await client.query('UPDATE merged_reports SET nos = nos + 1, updated_at = NOW() WHERE id = $1', [
        mergedReportId,
      ]);
    } else {
...
app.put('/api/admin/reports/:id', auth, adminAuth, async (req, res) => {
  ...
  try {
    ...
    if (reportCheck.rows[0].district !== district_name)
      return res.status(403).json({ message: 'Access denied. Report is not in your district.' });
    ...
    const reportsWithNames = await Promise.all(
      result.rows.map(async report => {
        ...
        const wardRes = await pool.query(
          'SELECT ward_no FROM wards WHERE ward_no = $1 AND district_id = (SELECT id FROM districts WHERE name = $2)',
          [report.ward, report.district],
        );
        return {
          ...report,
